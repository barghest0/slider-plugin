const changePosition = function (e: JQuery.MouseMoveEvent) {
	...
	let left =
		((e.pageX - $('.slider').position().left) / thisThumb.parentElement.size.width) * 100
	let top =
		((e.pageY - $('.slider').position().top) / thisThumb.parentElement.size.height) * 100
	...
	let deltaWidth = parseInt($(`.slider__thumb-1`).css('left'), 10) - parseInt($(`.slider__thumb-0`).css('left'), 10)
