...
function initializeInputs(this: Panel, root: string) {
	const panel = <HTMLElement>document.querySelector(`.js-${root.slice(1)}-panel`);
	...
	this.firstValueInput = <HTMLInputElement>panel.querySelector(`.js-${FIRST_VALUE_CLASS}`);
	this.secondValueInput = <HTMLInputElement>panel.querySelector(`.js-${SECOND_VALUE_CLASS}`);
	this.decimalPlacesInput = <HTMLInputElement>panel.querySelector(`.js-${DECIMAL_PLACES_CLASS}`);
